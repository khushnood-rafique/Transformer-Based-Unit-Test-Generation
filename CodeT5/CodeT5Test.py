...
model_path = 'results/codet5_finetuned'  # Path to the saved model
...
def generate_unit_tests(python_function, description, model, tokenizer, device, max_length=512):
    ...
    print(f"Before Inference: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
...
for  _, example in dataset.iterrows():
    ...
    reference_unit_test = example['Unit Test Cases'].strip()
