...
import black
from datasets import load_metric
from transformers import T5Tokenizer, T5ForConditionalGeneration,AutoModelForCausalLM,EncoderDecoderModel


dataset = pd.read_csv("data/test_data.csv")

# Load the fine-tuned model and tokenizer
model_path = 'results/codebert_finetuned'  # Path to the saved model
...
def generate_unit_tests(python_function, description, model, tokenizer, device, max_length=512):
    ...
    input_text = f"Function: {python_function} Description: {description}"
...
def format_predicted_unit_test(predicted):
    """
    Formats a predicted unit test by ensuring proper indentation 
    and handling 'assert' and 'self.assert*' cases correctly. 
    It also detects unittest structures and preserves them.
    
    Args:
        predicted (str): The raw predicted test case string.
        
    Returns:
        str: The properly formatted unit test.
    """
    # Split lines for easier processing
    lines = predicted.strip().split("\n")

    formatted_lines = []
    inside_test_function = False  # Track whether inside a test function

    for line in lines:
        stripped_line = line.strip()

        # Preserve `import unittest` and class declarations without modification
        if stripped_line.startswith("import unittest") or stripped_line.startswith("class "):
            formatted_lines.append(stripped_line)
            continue
        
        # Detect function definitions (test cases)
        if stripped_line.startswith("def "):
            formatted_lines.append(stripped_line)
            inside_test_function = True  # We're inside a test function now
            continue

        # Ensure assertions (both assert and self.assert*) are properly formatted
        if "assert" in stripped_line:
            if stripped_line.startswith("self.assert"):  # Keep self.assert* cases
                formatted_lines.append(f"    {stripped_line}")
            else:  # Handle plain assert cases
                formatted_lines.append(f"    assert {stripped_line.split('assert', 1)[1].strip()}")
            continue

        # Preserve other lines inside functions
        if inside_test_function:
            formatted_lines.append(f"    {stripped_line}")  # Indent function content properly
        else:
            formatted_lines.append(stripped_line)  # Keep outer-level lines as they are
...
for  _, example in dataset.iterrows():
    ...
    predicted_unit_test = format_predicted_unit_test(predicted_unit_test)
